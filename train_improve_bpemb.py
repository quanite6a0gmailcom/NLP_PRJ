from model_bpemb import build_transformer
from dataset_bpemb import BilingualDataset, causal_mask
...
def greedy_decode(model, source, source_mask, tokenizer_src, tokenizer_tgt, max_len, device):
    # 1. CẬP NHẬT CÚ PHÁP CỦA BPEMB
    sos_idx = tokenizer_tgt.spm.bos_id()
    eos_idx = tokenizer_tgt.spm.eos_id()
    ...
    while True:
        ...
        # 2. TỐI ƯU HÓA TỐC ĐỘ GPU
        # next_word đang là 1 Tensor trên GPU có shape [1].
        # Dùng .unsqueeze(1) để đổi thành [1, 1] và nối thẳng vào decoder_input.
        # Tuyệt đối không dùng .item() ở đây để tránh nghẽn cổ chai truyền dữ liệu về CPU.
        decoder_input = torch.cat(
            [decoder_input, next_word.unsqueeze(1)], dim=1
        )

        # Kiểm tra điểm dừng. Vì next_word là Tensor, ta có thể so sánh trực tiếp 
        # hoặc dùng .item() chỉ cho bước kiểm tra logic này.
...
def run_validation(model, validation_ds, tokenizer_src, tokenizer_tgt, max_len, device, print_msg, global_step, writer, num_examples=2):
    ...
    with torch.no_grad():
        for batch in validation_ds:
            ...
            model_out_text = tokenizer_tgt.decode_ids(model_out_ids)
            # THÊM 2 DÒNG NÀY ĐỂ "BỌC THÉP" ÉP KIỂU VỀ STRING
            if isinstance(model_out_text, list):
                model_out_text = " ".join(model_out_text)  # Lấy phần tử text đầu tiên ra khỏi list
    ...
    if writer:
        ...
        # 2. SỬA LỖI ĐỊNH DẠNG CER
        expected_cer = [e[0] for e in expected]
        metric_cer = torchmetrics.CharErrorRate()
        cer = metric_cer(predicted, expected_cer)
        writer.add_scalar('validation cer', cer, global_step)

        # 3. TÁI SỬ DỤNG BLEU-4 THAY VÌ TÍNH LẠI
        writer.add_scalar('validation BLEU', bleu4, global_step)
        writer.flush()
...
def get_or_build_tokenizer(config, ds, lang):
    ...
    if not Path.exists(tokenizer_path):
        if lang == 'ja' or lang == 'vi':
            tokenizer = Tokenizer(BPE(unk_token="[UNK]"))
            tokenizer.pre_tokenizer = Whitespace()
            trainer = BpeTrainer(special_tokens=["[UNK]", "[PAD]", "[SOS]", "[EOS]"],vocab_size=25000, min_frequency=1)
...
def ds_custom_val():
    # 1. Khai báo đường dẫn tới 2 file text của bạn
    # (Hãy thay đổi tên file và mã ngôn ngữ cho đúng với bài toán của bạn)
    file_src_path = 'dataset_bpemb/dev2010.ja-vi.ja' 
    file_tgt_path = 'dataset_bpemb/dev2010.ja-vi.vi'
...
def ds_custom_test():
    # 1. Khai báo đường dẫn tới 2 file text của bạn
    # (Hãy thay đổi tên file và mã ngôn ngữ cho đúng với bài toán của bạn)
    file_src_path = 'dataset_bpemb/tst2010.ja-vi.ja' 
    file_tgt_path = 'dataset_bpemb/tst2010.ja-vi.vi'
...
def get_ds(config):
    ...
    print(f"Kích thước từ vựng: {tokenizer_tgt.vocab_size}")
    ...
    for item in ds_raw:
        # Cập nhật API lấy ID (Giả định cả 2 đều dùng BPEmb)
        src_ids = tokenizer_src.encode_ids(item['translation'][config['lang_src']])
        tgt_ids = tokenizer_tgt.encode_ids(item['translation'][config['lang_tgt']])
        
        # NẾU tokenizer_src vẫn là bộ cũ, hãy mở comment dòng dưới và xóa dòng src_ids ở trên:
...
def train_model(config):
    ...
    # Sửa thành tokenizer_tgt và dùng cú pháp của BPEmb
    loss_fn = nn.CrossEntropyLoss(
        ignore_index=tokenizer_tgt.spm.unk_id(), 
        label_smoothing=0.1
    ).to(device)
